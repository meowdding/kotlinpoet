...
import me.owdding.kotlinpoet.KModifier.ABSTRACT
import me.owdding.kotlinpoet.KModifier.DATA
import me.owdding.kotlinpoet.KModifier.IN
import me.owdding.kotlinpoet.KModifier.INNER
import me.owdding.kotlinpoet.KModifier.INTERNAL
import me.owdding.kotlinpoet.KModifier.PRIVATE
import me.owdding.kotlinpoet.KModifier.PUBLIC
import me.owdding.kotlinpoet.KModifier.SEALED
...
import me.owdding.kotlinpoet.jvm.throws
...
class TypeSpecTest {
  private val tacosPackage = "me.owdding.tacos"
  ...
  @Test fun anonymousClassWithMultipleSuperTypes() {
    val superclass = ClassName("me.owdding.wire", "Message")
    val anonymousClass = TypeSpec.anonymousClassBuilder()
      .superclass(superclass)
  ...
  @Test fun typeVariableWithBounds() {
    val a = AnnotationSpec.builder(ClassName("me.owdding.tacos", "A")).build()
    val p = TypeVariableName("P", Number::class)
  ...
  @Test fun classImplementsExtends() {
    ...
    val food = ClassName("me.owdding.tacos", "Food")
  ...
  @Test fun funInterfaceExtendsFunInterfaceWithoutMethod() {
    val food = ClassName("me.owdding.food", "Food")
    val taco = ClassName(tacosPackage, "Taco")

    val source = FileSpec.builder("me.owdding.food", "Food")
      .addType(
  ...
  @Test fun referencedAndDeclaredSimpleNamesConflict() {
    ...
    assertThat(toString(top)).isEqualTo(
      """
        |package me.owdding.tacos
        |
        |import me.owdding.donuts.Bottom
        |
        |public class Top {
        ...
        |
        |  public val externalTop: me.owdding.donuts.Top
        |
        ...
        |    public val externalTop: me.owdding.donuts.Top
        |
        |    public val externalBottom: me.owdding.donuts.Bottom
        |
        |    public class Bottom {
        |      public val internalTop: Top
        |
  ...
  @Test fun simpleNamesConflictInThisAndOtherPackage() {
    ...
    assertThat(toString(gen)).isEqualTo(
      """
        ...
        |
        |  public val externalOther: me.owdding.donuts.Other
        |}
        |
      """.trimMargin(),
    )
  ...
  @Test fun constructorToString() {
    ...
    assertThat(constructor.toString()).isEqualTo(
      "" +
        "public constructor(taco: me.owdding.tacos.Taco) {\n" +
        "  this.taco = taco\n" +
        "}\n",
    )
  }
  ...
  @Test fun parameterToString() {
    ...
    assertThat(parameter.toString())
      .isEqualTo("@javax.`annotation`.Nullable crossinline taco: me.owdding.tacos.Taco")
  }
    ...
    val taco = TypeSpec.classBuilder("Taco")
      .addFunction(
        FunSpec.builder("addTopping")
          .addParameter("topping", ClassName("me.owdding.tacos", "Topping"))
          .beginControlFlow("try")
          .addCode("/* do something tricky with the topping */\n")
          .nextControlFlow(
            "catch (e: %T)",
            ClassName("me.owdding.tacos", "IllegalToppingException"),
          )
          .endControlFlow()
          .build(),
      )
  ...
  fun classHeaderAnnotations() {
    val idParameterSpec = ParameterSpec.builder("id", Int::class)
      .addAnnotation(ClassName("me.owdding.kotlinpoet", "Id"))
      .defaultValue("1")
      .build()
    ...
    val typeSpec = TypeSpec.classBuilder("Person")
      ...
      .addProperty(
        PropertySpec.builder("id", Int::class)
          .addModifiers(PRIVATE)
          .initializer("id")
          .addAnnotation(ClassName("me.owdding.kotlinpoet", "OrderBy"))
          .build(),
      )
      .build()
    ...
    assertThat(toString(typeSpec)).isEqualTo(
      ...
      |package me.owdding.tacos
      |
      |import me.owdding.kotlinpoet.Id
      |import me.owdding.kotlinpoet.OrderBy
      |import kotlin.Int
      |
      |public data class Person(
      |  @OrderBy
...
// https://github.com/square/kotlinpoet/issues/2033
    ...
    val source = FileSpec.builder("me.owdding.tacos", "Taco")
      .addType(
        TypeSpec.classBuilder("Taco")
          ...
          .addFunction(
            FunSpec.constructorBuilder()
              .addParameter("anotherTaco", ClassName("me.owdding.tacos", "Taco"))
  ...
  @Test fun escapeInnerClassName() {
    val tacoType = ClassName("me.owdding.tacos", "Taco", "object")
    ...
    assertThat(funSpec.toString()).isEqualTo(
      """
      |public fun printTaco(taco: me.owdding.tacos.Taco.`object`) {
      |  print(taco)
      |}
      |
      """.trimMargin(),
  ...
  @Test fun primaryConstructorCanNotDelegate() {
    assertThrows<IllegalArgumentException> {
      TypeSpec.classBuilder("Child")
        .superclass(ClassName("me.owdding", "Parent"))
        .primaryConstructor(
          FunSpec.constructorBuilder()
            .callSuperConstructor(CodeBlock.of("%L", "param"))
            .addParameter(
              name = "param",
              type = ClassName("me.owdding", "Param"),
            )
            .build(),
        )
        .build()
  ...
  companion object {
    private const val donutsPackage = "me.owdding.donuts"
  }
}
