...
import me.owdding.kotlinpoet.KModifier.ANNOTATION
import me.owdding.kotlinpoet.KModifier.COMPANION
import me.owdding.kotlinpoet.KModifier.ENUM
import me.owdding.kotlinpoet.KModifier.EXPECT
import me.owdding.kotlinpoet.KModifier.EXTERNAL
import me.owdding.kotlinpoet.KModifier.FUN
import me.owdding.kotlinpoet.KModifier.INLINE
import me.owdding.kotlinpoet.KModifier.INTERNAL
import me.owdding.kotlinpoet.KModifier.PRIVATE
import me.owdding.kotlinpoet.KModifier.PROTECTED
import me.owdding.kotlinpoet.KModifier.PUBLIC
import me.owdding.kotlinpoet.KModifier.SEALED
import me.owdding.kotlinpoet.KModifier.VALUE
...
public class TypeSpec private constructor(
    ...
    try {
      if (enumName != null) {
        ...
        val supertype = if (superclass != ANY) {
          if (!areNestedExternal && !modifiers.contains(KModifier.EXPECT)) {
  ...
  public class Builder internal constructor(
    ...
    internal val isCompanion get() = kind == Kind.OBJECT && KModifier.COMPANION in modifiers
    ...
    internal val isFunInterface get() = kind == Kind.INTERFACE && KModifier.FUN in modifiers
    ...
    private fun checkCanHaveInitializerBlocks() {
      ...
      check(KModifier.EXPECT !in modifiers) {
    ...
    override fun addProperty(propertySpec: PropertySpec): Builder = apply {
      if (KModifier.EXPECT in modifiers) {
    ...
    public fun build(): TypeSpec {
      ...
      for (funSpec in funSpecs) {
        ...
        when {
          ...
          KModifier.EXPECT in modifiers -> require(funSpec.body.isEmpty()) {
  ...
  public companion object {
    ...
    @JvmStatic
    public fun expectClassBuilder(name: String): Builder = Builder(Kind.CLASS, name, KModifier.EXPECT)
    ...
    @JvmStatic
    public fun expectClassBuilder(className: ClassName): Builder = classBuilder(className.simpleName).addModifiers(
      KModifier.EXPECT
    )
    ...
    public fun companionObjectBuilder(name: String? = null): Builder =
      Builder(Kind.OBJECT, name, KModifier.COMPANION)
    ...
    @JvmStatic public fun funInterfaceBuilder(name: String): Builder =
      Builder(Kind.INTERFACE, name, KModifier.FUN)
